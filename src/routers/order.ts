...
// user types
import { orderBodyReguest } from "../types/order";
...
const orderRouters = async (app: FastifyInstance) => {
  ...
  app.post(
    "/",
    { preHandler: [verifyUserAuth], schema: orderCreateSchema },
    createOrder
  ); // Order create
  ...
  app.get("/orders", {}, getOrderListHomePage);
};

// ******Order handlers*******
async function createOrder(req: orderBodyReguest) {
  const { order_type, title, price, amount, cost } = req.body;
  return await this.orderHandlers.createOrder(
    order_type,
    title,
    price,
    amount,
...
async function getOrderListHomePage() {
